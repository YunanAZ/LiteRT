...
class EnvironmentOptions : public internal::Handle<LiteRtEnvironmentOptions,
                                                   internal::DummyDeleter> {
 public:
  // EnvironmentOptions are always owned by somne environment, this can never be
  // an owning handle.
  explicit EnvironmentOptions(LiteRtEnvironmentOptions env)
      : Handle(env, OwnHandle::kNo) {}
