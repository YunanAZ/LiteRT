...
TEST(LiteRtQualcommOptionsTest, LogLevel) {
  ...
  LITERT_ASSERT_OK(LiteRtQualcommOptionsSetLogLevel(qualcomm_options,
                                                    kQualcommLogLevelWarn));

  LiteRtQualcommOptionsLogLevel log_level;
  LITERT_ASSERT_OK(
      LiteRtQualcommOptionsGetLogLevel(qualcomm_options, &log_level));
  EXPECT_EQ(log_level, kQualcommLogLevelWarn);
...
TEST(QualcommOptionsTest, CppApi) {
  ...
  EXPECT_EQ(options.GetLogLevel(), kQualcommLogLevelInfo);
  options.SetLogLevel(kQualcommLogLevelWarn);
  EXPECT_EQ(options.GetLogLevel(), kQualcommLogLevelWarn);
...
TEST(QualcommOptionsTest, FromOpaque) {
  ...
  EXPECT_TRUE(QualcommOptions::FromOpaque(*options));
}

TEST(QualcommOptionsTest, FromOpaqueNotFound) {
  void* payload = malloc(8);
  auto options =
      OpaqueOptions::Create("not-qualcomm", payload, [](void* d) { free(d); });
  auto qnn_options = QualcommOptions::Create();

  EXPECT_FALSE(QualcommOptions::FromOpaque(*options));
}
