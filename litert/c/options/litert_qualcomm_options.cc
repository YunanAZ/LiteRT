...
struct LiteRtQualcommOptionsT {
  LiteRtQualcommOptionsLogLevel log_level = kQualcommLogLevelInfo;
...
LiteRtStatus LiteRtQualcommOptionsGet(LiteRtOpaqueOptions options,
  ...
  if (absl::string_view(identifier) != LiteRtQualcommOptionsGetIdentifier()) {
    return kLiteRtStatusErrorInvalidArgument;
  }
...
LiteRtStatus LiteRtQualcommOptionsGetEnableWeightSharing(
    LiteRtQualcommOptions options, bool* enable_weight_sharing) {
  if (enable_weight_sharing == nullptr) {
    return kLiteRtStatusErrorInvalidArgument;
  }
...
QualcommOptions QualcommOptions::Create() {
  LiteRtOpaqueOptions options;
  internal::AssertOk(LiteRtQualcommOptionsCreate, &options);
  return QualcommOptions(options, kYes);
}
...
Expected<QualcommOptions> QualcommOptions::FromOpaque(OpaqueOptions& options) {
  if (options.GetIdentifier() &&
      *options.GetIdentifier() == QualcommOptions::GetIdentifier()) {
    return QualcommOptions(options.Get(), kNo);
  }
  Expected<OpaqueOptions> chain = options.Next();
  while (chain) {
    if (chain->GetIdentifier() &&
        *chain->GetIdentifier() == QualcommOptions::GetIdentifier()) {
      return QualcommOptions(chain->Get(), kNo);
    }
    chain = chain->Next();
  }
